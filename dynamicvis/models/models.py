try:
    from mmcv.cnn.bricks.transformer import nlc_to_nchw, nchw_to_nlc
except Exception:
    import torch
    def nchw_to_nlc(x: torch.Tensor):
        return x.flatten(2).transpose(1, 2).contiguous()
    def nlc_to_nchw(x: torch.Tensor, hw_shape):
        H, W = hw_shape
        return x.transpose(1, 2).reshape(x.size(0), -1, H, W).contiguous()

# ---- safe, lazy FPN import to avoid circular mmdet<->dynamicvis -----
MMDET_FPN = None
def get_mmdet_fpn():
    global MMDET_FPN
    if MMDET_FPN is None:
        try:
            # 优先精确导入，不触发 mmdet.models 顶层的广域导入
            from importlib import import_module
            MMDET_FPN = import_module('mmdet.models.necks.fpn').FPN
        except Exception:
            # 兜底：再尝试从 mmdet.models 拿（此时大概率已完成初始化）
            from mmdet.models import FPN as _FPN
            MMDET_FPN = _FPN
    return MMDET_FPN
...
try:
    ...
    # Fallback: local implementations to avoid circular import / missing symbols
    import torch
    def nchw_to_nlc(x: torch.Tensor):
        # (N,C,H,W) -> (N,HW,C)
        return x.flatten(2).transpose(1, 2).contiguous()
...
@CDMODELS.register_module()
class DualInputsFPN(get_mmdet_fpn()):
