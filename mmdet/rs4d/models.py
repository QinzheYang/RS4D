...
from typing import List, T, Tuple, Optional, Dict, Union, Sequence
...
class RSPrompterAnchor(MaskRCNN):
    def __init__(
        ...
        self.frozen_modules = []
        if peft_config is None and backbone_freeze:
    ...
    def extract_feat(self, batch_inputs: Tensor) -> Tuple[Tensor]:
        ...
        if isinstance(vision_outputs, SamVisionEncoderOutput):
            ...
            # BHWC -> BCHW
            image_embeddings = vision_outputs[-1].permute(0, 3, 1, 2)
...
class RSMask2FormerHead(Mask2FormerHead, BaseModule):
    ...
    def _forward_head(self, decoder_out: Tensor, mask_feature: Tensor,
        ...
        image_embeddings = image_embeddings.unsqueeze(0).repeat(self.num_queries, 1, 1, 1)
        ...
        #image_positional_embeddings = image_positional_embeddings.to(torch.device('cpu'))
        mask_pred, iou_predictions, mask_dencoder_attentions = self.mask_decoder(
            image_embeddings=image_embeddings,#.to(torch.device('cpu')),
            image_positional_embeddings=image_positional_embeddings,#.to(torch.device('cpu')),
            sparse_prompt_embeddings=sparse_embeddings,#.to(torch.device('cpu')),
            dense_prompt_embeddings=dense_embeddings,#.to(torch.device('cpu')),
            multimask_output=self.multimask_output,#.to(torch.device('cpu')),
            attention_similarity=self.attention_similarity,#.to(torch.device('cpu')),
            target_embedding=self.target_embedding,#.to(torch.device('cpu')),
            output_attentions=self.output_attentions,#.to(torch.device('cpu')),
        )
        #mask_pred = mask_pred.reshape(img_bs, -1, *mask_pred.shape[-2:])
        height, width = mask_pred.shape[-2], mask_pred.shape[-1]

        # 生成新的张量，形状为 (img_bs, -1, height, width)
        mask_pred = torch.randn(img_bs, 30, height, width)
    ...
    def forward(self, x: List[Tensor],
        ...
        #print("??????????????")
        #print(image_positional_embeddings)
        #print(len(x))
        #print(len(x[0]))
        #print(x[0][0])
        #print("????????????????????????????????????")
        ...
        mask_features, multi_scale_memorys = self.pixel_decoder(x[0])
        ...
        mask_pred_plus_list = []
        attn_mask = None
        image_embeddings=x[1][0]
        image_positional_embeddings=x[2][0]
...
@MODELS.register_module()
class RSSamVisionEncoder_prue(BaseModule):
    def __init__(
            self,
            hf_pretrain_name,
            extra_config=None,
...
@MODELS.register_module()
class RSMamba(BaseModule):
    arch_zoo = {
        **dict.fromkeys(
            ['s', 'small'], {
                'embed_dims': 256,#768 // 4,
                'num_layers': 8*2,
                'feedforward_channels': 256*2,#768 // 2,
            }),
        **dict.fromkeys(
            ['b', 'base'], {
                'embed_dims': 256,
        ...
        **dict.fromkeys(
            ['l', 'large'], {
                'embed_dims': 256,#256,#320,#320,#1024//4,
                'num_layers': 28,#14*2,
                'feedforward_channels': 640,#640,#1024//2
            }),
        **dict.fromkeys(
            ['h', 'huge'],
            {
                'embed_dims': 256,#1280//4,
                'num_layers': 32,
    ...
    OUT_TYPES = {'featmap', 'avg_featmap', 'cls_token', 'raw', 'featmap_hwc'}
    ...
    def __init__(self,
        ...
        self.pre_norm = build_norm_layer_pretrain(norm_cfg, self.embed_dims)

        self.final_norm = final_norm
        if final_norm:
            self.ln1 = build_norm_layer_pretrain(norm_cfg, self.embed_dims)
        if self.out_type == 'avg_featmap':
            self.ln2 = build_norm_layer_pretrain(norm_cfg, self.embed_dims)
        # freeze stages only when self.frozen_stages > 0
        if self.frozen_stages > 0:
            self._freeze_stages()
        #self.load_state_dict(torch.load('/mnt/user/yangqinzhe/project/project_code/RSPrompter-mamba/base_model_parameterskk.pth'))

        #self.load_state_dict(torch.load('/mnt/user/yangqinzhe/project/project_code/RSPrompter-mamba/noise_model_parameterskk.pth'))

        #self.load_state_dict(torch.load('/mnt/user/yangqinzhe/project/project_code/RSPrompter-mamba/model_parameterskk.pth'))#2% 默认这个

        #self.load_state_dict(torch.load('/mnt/user/yangqinzhe/project/project_code/RSPrompter-mamba/models_after_dis/20241217-032552_vim_scan2_1024_vitbase_l1_noise_10_0/model_parameterskk.pth'))
        #self.load_state_dict(torch.load('/mnt/user/yangqinzhe/project/project_code/RSPrompter-mamba/models_after_dis/20241217-033050_vim_scan2_1024_vitbase_l1_noise_0_10/model_parameterskk.pth'))
        #self.load_state_dict(torch.load('/mnt/user/yangqinzhe/project/project_code/RSPrompter-mamba/models_after_dis/20241217-033050_vim_scan2_1024_vitbase_l1_noise_0_10/model_parameterskk.pth'))

        #self.load_state_dict(torch.load('/mnt/user/yangqinzhe/project/project_code/RSPrompter-mamba/models_after_dis/20241217-031933_vim_scan2_1024_vitbase_l1_noise_30_30/model_parameterskk.pth'))
        #self.load_state_dict(torch.load('/mnt/user/yangqinzhe/project/project_code/RSPrompter-mamba/models_after_dis/20241217-091038_vim_scan2_1024_vitbase_l1_noise_large_28_256/model_parameterskk.pth'))
        #self.load_state_dict(torch.load('/mnt/user/yangqinzhe/project/project_code/RSPrompter-mamba/models_after_dis/20241217-090406_vim_scan2_1024_vitbase_l1_noise_huge_32_256/model_parameterskk.pth'))
        #self.load_state_dict(torch.load(''))

        #self.load_state_dict(torch.load('/mnt/user/yangqinzhe/project/project_code/RSPrompter-mamba/models_after_dis/20241217-091915_vim_scan2_1024_vitbase_l1_noise_large_48_256/model_parameterskk.pth'))

        #self.load_state_dict(torch.load('/mnt/dataset/yangqinzhe/Rsprompter-mamba-pth/new/work_dirs/rsprompter-test/samseg-maskrcnn-mamba-nwpu-noise/best_coco_bbox_mAP_epoch_2.pth'))

        #20%#self.load_state_dict(torch.load('/mnt/user/yangqinzhe/project/project_code/RSPrompter-mamba/models_after_dis/20250123-074529_vim_scan2_1024_vitbase_l1_noise_20_20/model_parameterskk.pth'))

        #20%#self.load_state_dict(torch.load('/mnt/user/yangqinzhe/project/project_code/RSPrompter-mamba/models_after_dis/20250126-024810_vim_scan2_1024_vitbase_l1_noise_20_20/model_parameterskk.pth'))

        #self.load_state_dict(torch.load('/mnt/user/yangqinzhe/project/project_code/RSPrompter-mamba/models_after_dis/20250226-141356_vim_scan2_1024_vitbase_l1_noise_20_20/model_parameterskk.pth'))#10k训一遍

        #self.load_state_dict(torch.load('/mnt/user/yangqinzhe/project/project_code/RSPrompter-mamba/10xmodels_after_dis/20250227-053855_vim_scan2_1024_vitbase_l1_noise_20_20/model_parameterskk.pth'))  # 10k训一遍

        #state_dict = torch.load('/mnt/user/yangqinzhe/project/project_code/RSPrompter-mamba/10xmodels_after_dis/20250227-053855_vim_scan2_1024_vitbase_l1_noise_20_20/model_parameterskk.pth')#100k一次，4卡
        #state_dict = torch.load('/mnt/user/yangqinzhe/project/project_code/RSPrompter-mamba/10xmodels_after_dis/20250227-022657_vim_scan2_1024_vitbase_l1_noise_20_20/model_parameterskk.pth')#100k一次，单卡
        state_dict = torch.load('/mnt/user/yangqinzhe/project/project_code/RSPrompter-mamba/10xmodels_after_dis/20250227-021854_vim_scan2_1024_vitbase_l1_noise_20_20_500k/model_parameterskk.pth')#500k一次，4卡
        #state_dict = torch.load('/mnt/user/yangqinzhe/project/project_code/RSPrompter-mamba/models_after_dis/20250227-022141_vim_scan2_1024_vitbase_l1_noise_20_20_500k/model_parameterskk.pth')  # 500k一次，单卡
        #state_dict = torch.load('/mnt/user/yangqinzhe/project/project_code/RSPrompter-mamba/models_after_dis/20250402-073530_vim_scan2_1024_vitbase_l1_noise_20_20_500k_geo3/model_parameterskk.pth')  # geo3
        #state_dict = torch.load('/mnt/user/yangqinzhe/project/project_code/RSPrompter-mamba/models_after_dis/20250402-073006_vim_scan2_1024_vitbase_l1_noise_20_20_500k_geo1/model_parameterskk.pth')  # geo1
        #state_dict = torch.load('/mnt/user/yangqinzhe/project/project_code/RSPrompter-mamba/models_after_dis/20250402-022816_vim_scan2_1024_vitbase_l1_noise_20_20_500k_geo4/model_parameterskk.pth')  # geo4
        #state_dict = torch.load('/mnt/user/yangqinzhe/project/project_code/RSPrompter-mamba/models_after_dis/20250402-020957_vim_scan2_1024_vitbase_l1_noise_20_20_500k_geo2/model_parameterskk.pth')  # geo2
        #state_dict = torch.load('/mnt/user/yangqinzhe/project/project_code/RSPrompter-mamba/10xmodels_after_dis/20251203-023234_vim_scan2_1024_vitbase_l1_noise_20_20_20w/model_parameterskk.pth')


        #创建新的 state dict，去掉 'module.' 前缀
        new_state_dict = {}
    ...
    def _format_output(self, x, hw):
        ...
        if self.out_type == 'featmap_hwc':
            B = x.size(0)
            return patch_token.reshape(B, *hw, -1)
        if self.out_type == 'avg_featmap':
...
class RSFeatureAggregator(BaseModule):
    in_channels_dict = {
        ...
        'large': [1024] * (24+1),
        'huge': [1280] * (32+1),
        'rsmamba_small': [256] * 16,
        'rsmamba_base': [256] * 24,
        'rsmamba_large': [256] * 28,#256,28
        'rsmamba_large1': [320] * 24,
        'rsmamba_huge': [256] * 32,
        'rsmamba_uhuge2': [256] * 48,
    }
...
@MODELS.register_module()
class RSFeaturePaAggregator(BaseModule):
    in_channels_dict = {
        'base': [768] * (12+1),
        'large': [1024] * (24+1),
        'huge': [1280] * (32+1),
    ...
    def forward(self, inputs):
        ...
        # 反向操作: 将融合的 x 加到倒数的 hidden_state，并逐层向前传递
        for idx in range(len(features) - 1, 0, -1):
            hidden_state = features[idx]
            # 先更新 x
            hidden_state_after = self.reverse_convs[idx](x)  # 应使用 reverse_convs 处理 x
            x = hidden_state_after + x

            # 将当前的 x 加到上一个 hidden_state
            hidden_state = features[idx - 1]
            x = self.reverse_convs[idx - 1](hidden_state) + x
...
@MODELS.register_module()
class RSSimplePaFPN(BaseModule):
    def __init__(self,
                 backbone_channel: int,
                 in_channels: List[int],
                 out_channels: int,
    ...
    def forward(self, input: Tensor) -> Tuple[Tensor]:
        ...
        # build outputs and fuse feature maps
        outs = [self.fpn_convs[i](laterals[i]) for i in range(self.num_ins)]

        # fuse feature maps as per PAFPN strategy
        for i in range(1, len(outs)):
            # 使用卷积将前一层的输出调整为当前层的尺寸和通道数
            fused = F.interpolate(outs[i-1], size=outs[i].shape[2:], mode='bilinear', align_corners=False)
            outs[i] = outs[i] + fused

            # 对融合后的特征进行卷积，保持通道数与输出一致
            outs[i] = self.fpn_convs[i](outs[i])  # 可以使用fused conv来处理

        # part 2: add extra levels if needed
        if self.num_outs > len(outs):
            for i in range(self.num_outs - self.num_ins):
                outs.append(F.max_pool2d(outs[-1], 1, stride=2))
